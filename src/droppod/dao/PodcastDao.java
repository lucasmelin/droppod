...
public class PodcastDao {
        ...
        public static boolean updatePodcastEpisodes(List<SyndEntry> episodes,PodcastModel podcast) {
            ...
            try {
                ...
                pst = conn.prepareStatement("select * from droppod.episodes where podcast_id = ? order by release_date desc");
                ...
                if(rs.next()) {
                    ...
                    pstEpisodeInfo = conn.prepareStatement("INSERT IGNORE INTO droppod.episodes (url, podcast_id, release_date)" + "VALUES  (?, ?, ?)");
                    pstEpisodeTran = conn.prepareStatement("INSERT IGNORE INTO droppod.episodes_translations (episode_id, language_code, episode_name, episode_description) VALUES (LAST_INSERT_ID(), ?, ?, ?)");
                    for(int j=0;j<episodes.size();j++) {
                    SyndEntry ep = episodes.get(j);
                        pstEpisodeInfo.setString(1, ep.getEnclosures().get(0).getUrl());
                        pstEpisodeInfo.setInt(2, podId);
                        pstEpisodeInfo.setTimestamp(3,
